...
void CppDataModel::loadData() {
  ...
  dataLoaded(dummyData);
}

QString CppDataModel::createRandomData() {
  QString dummyData = "[";
  dummyData.append(createRandomEntry(2017,"Burghausen")).append(",");
...
QString CppDataModel::createRandomEntry(int year, const QString& city) {
  return "{ \"year\": \"" + QString::number(year) + "\", \"city\": \"" + city + "\", \"expenses\": " + QString::number(qrand() % 5000) +" }";
}
